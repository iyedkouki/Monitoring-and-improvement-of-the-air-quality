...
public class TemperatureYear extends AppCompatActivity {
    RecyclerView recyclerView;
    ArrayList<YearClass> yearClasses;
    TemperatureYearAdapter humidityYearAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_temperature_year);
        recyclerView=findViewById(R.id.recycler);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(TemperatureYear.this));
        lineChart=findViewById(R.id.lineChart);
        retourne=findViewById(R.id.returne_);
        retourne.setOnClickListener(view -> startActivity(new Intent(TemperatureYear.this, Temperature_main.class)));
        db= FirebaseFirestore.getInstance();
        yearClasses =  new ArrayList<YearClass>();
        humidityYearAdapter =new TemperatureYearAdapter(TemperatureYear.this, yearClasses);
        recyclerView.setAdapter(humidityYearAdapter);
        EvenChangelister();


        buttond=findViewById(R.id.button);
        buttond.setOnClickListener(view -> startActivity(new Intent(TemperatureYear.this, TemperatureDay.class)));
        buttonm=findViewById(R.id.buttonm);
        buttonm.setOnClickListener(view -> startActivity(new Intent(TemperatureYear.this, TemperatureMonth.class)));
   ...
   /* private void updateGraphWithRecyclerViewData() {
        ...
        for (int i = 0, j = yearClasses.size() - 1; i < yearClasses.size(); i++, j--) {
            ...
            // Use the index as X-axis value (for simplicity) and PressureMax as Y-axis value
            entries.add(new Entry(i, Float.parseFloat(yearClass.gettempa())));
    ...
    private void updateGraphWithRecyclerViewData() {
        ...
        // Iterate through the yearClasses ArrayList to get document names and TemperatureAvg values
        for (int i = 0, j = yearClasses.size() - 1; i < yearClasses.size(); i++, j--) {
            YearClass yearClass = yearClasses.get(j);

            // Check if the temperature data is not null and can be parsed
            String tempString = yearClass.gettempa();
            if (tempString != null && !tempString.isEmpty()) {
                try {
                    float temperature = Float.parseFloat(tempString.trim());
                    entries.add(new Entry(i, temperature));
                } catch (NumberFormatException e) {
                    Log.e("DataError", "Error parsing temperature value: " + tempString);
                    // Skip this entry or set a default value
                    entries.add(new Entry(i, 0)); // You can decide what default value to set
                }
            } else {
                Log.e("DataError", "Temperature value is null or empty");
                // Handle missing or null temperature data (e.g., set to default value)
                entries.add(new Entry(i, 0)); // Default value for missing temperature
            }
            labels.add(yearClass.getcurrentanne()); // Use currentYear for X-axis labels
        }
        ...
        // Enable zoom and set initial zoom level to show only the last value
        lineChart.moveViewToX(entries.size() - 1);
        lineChart.setVisibleXRangeMaximum(12); // Show only a limited range of entries
