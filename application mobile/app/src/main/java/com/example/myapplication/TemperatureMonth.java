...
public class TemperatureMonth extends AppCompatActivity {
    RecyclerView recyclerView;
    ArrayList<MonthClass> monthClasses;
    TemperatureMonthAdapter humidityMonthAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_temperature_month);

        recyclerView=findViewById(R.id.recycler);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(TemperatureMonth.this));
        lineChart=findViewById(R.id.lineChart);
        retourne=findViewById(R.id.returne_);
        retourne.setOnClickListener(view -> startActivity(new Intent(TemperatureMonth.this, Temperature_main.class)));
        db= FirebaseFirestore.getInstance();
        monthClasses=  new ArrayList<MonthClass>();
        humidityMonthAdapter=new TemperatureMonthAdapter(TemperatureMonth.this,monthClasses);
        recyclerView.setAdapter(humidityMonthAdapter);
        EvenChangelister();

        buttond=findViewById(R.id.button);
        buttond.setOnClickListener(view -> startActivity(new Intent(TemperatureMonth.this, TemperatureDay.class)));
        buttony=findViewById(R.id.buttony);
        buttony.setOnClickListener(view -> startActivity(new Intent(TemperatureMonth.this, TemperatureYear.class)));
    ...
    private void updateGraphWithRecyclerViewData() {
        ...
        for (int i = 0, j = monthClasses.size() - 1; i < monthClasses.size(); i++, j--) {
            ...
            // Use the index as X-axis value (for simplicity) and PressureMax as Y-axis value
            entries.add(new Entry(i, Float.parseFloat(monthClass.gettempm())));
